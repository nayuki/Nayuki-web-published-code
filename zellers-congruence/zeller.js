...
function dayOfWeek(y, m, d) {
    ...
    m %= 12;
    d = d % 7 + 7;
    const temp = y + div(y, 4) - div(y, 100) + div(y, 400);
...
const TEST_SUITE = [
    ...
    function testVsNaiveRandomly() {
        const TRIALS = 1000;
        for (let i = 0; i < TRIALS; i++) {
            const y = Math.floor(Math.random() * 800) + 1600;
            const m = Math.floor(Math.random() * 12) + 1;
            const d = Math.floor(Math.random() * monthLength(y, m)) + 1;
            assertEquals(dayOfWeekNaive(y, m, d), dayOfWeek(y, m, d));
        }
    },
    function testLenientExtreme() {
        const CASES = [
    ...
    function testLenientRandomly() {
        const TRIALS = 1000000;
        for (let i = 0; i < TRIALS; i++) {
            let y = Math.floor(Math.random() * 400) + 2000;
            let m = Math.floor(Math.random() * 12) + 1;
            let d = Math.floor(Math.random() * monthLength(y, m)) + 1;
            const dow = dayOfWeek(y, m, d);
            const temp = Math.floor(Math.random() * 10000) - 5000;
            y += temp;
            m -= temp * 12;
...
const MONTH_LENGTHS = [-1, 31, -1, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
