...
(function (app) {
    ...
    ConventionalEnglishNotation.ONES = [
        "", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine",
        "ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", "seventeen", "eighteen", "nineteen"
    ];
    ConventionalEnglishNotation.TENS = [
        "", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"
    ];
    ConventionalEnglishNotation.ILLIONS = [
    ...
    class YllionEnglishNotation {
        static numberToWords(n) {
            ...
            // 0 <= n <= 99, borrow functionality from another class
                ...
                const yllionsLen = YllionEnglishNotation.YLLIONS.length;
                if (temp.length > (1 << yllionsLen))
                    throw new RangeError("Number too large");
                for (let i = yllionsLen - 1; i >= 1; i--) {
                    if (temp.length > (1 << i)) {
                        const split = temp.length - (1 << i);
                        const high = BigInt(temp.substring(0, split));
                        const low = BigInt(temp.substring(split));
        ...
        static toStringWithSeparators(n) {
            if (n < 0n)
                return "-" + YllionEnglishNotation.toStringWithSeparators(-n);
            else {
                let s = n.toString();
                for (let i = s.length - 4, j = 1; i > 0; i -= 4, j++) {
                    const k = Math.min(YllionEnglishNotation.numTrailingZeros(j), YllionEnglishNotation.SEPARATORS.length - 1);
                    s = s.substring(0, i) + YllionEnglishNotation.SEPARATORS[k] + s.substring(i);
                }
                return s;
            }
        }
        static numTrailingZeros(n) {
    ...
    class YllionChineseNotation {
        static numberToWords(n) {
            if (n < 0n)
                ...
                const temp = n.toString();
                const yllionsLen = YllionChineseNotation.YLLIONS.length;
                if (temp.length > (1 << yllionsLen))
                    throw new RangeError("Number too large");
                for (let i = yllionsLen - 1; i >= 1; i--) {
                    if (temp.length > (1 << i)) {
    ...
    YllionChineseNotation.ONES = ["", "\u4E00", "\u4E8C", "\u4E09", "\u56DB", "\u4E94", "\u516D", "\u4E03", "\u516B", "\u4E5D"];
    YllionChineseNotation.YLLIONS = ["", "\u767E", "\u842C", "\u5104", "\u5146", "\u4EAC", "\u5793", "\u79ED", "\u7A70", "\u6E9D", "\u6F97", "\u6B63", "\u8F09"];
})(app || (app = {}));
