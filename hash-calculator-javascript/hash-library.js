...
(function (hashlib) {
    ...
    class BlockHasher {
        ...
        update(buf) {
            ...
            if (this.blockFilled > 0) {
                const n = Math.min(blockLen - this.blockFilled, b.length);
    ...
    Crc32Hasher.TABLE = new Uint32Array(256);
    (() => {
    ...
    /*---- SHA-1 ----*/
    hashlib.Sha1 = new class {
        getName() { return "SHA-1"; }
        getHashLength() { return 20; }
        newHasher() { return new BlockHasher(64, new Sha1Core()); }
    };
    class Sha1Core {
        constructor() {
        ...
        getHashDestructively(bh, totalLength) {
    ...
    /*---- SHA-256 ----*/
    hashlib.Sha256 = new class {
        getName() { return "SHA-256"; }
        getHashLength() { return 32; }
        newHasher() { return new BlockHasher(64, new Sha256Core()); }
    };
    class Sha256Core {
        constructor() {
        ...
        compressBlocks(msg) {
            ...
            for (let i = 0; i < msg.length;) {
                for (let j = 0; j < 16; j++, i += 4)
                    schedule[j] = view.getUint32(i);
                for (let j = 16; j < schedule.length; j++) {
                    let temp = (schedule[j - 16] + schedule[j - 7]) | 0;
                    const x = schedule[j - 15];
                    temp = (temp + ((x << 25 | x >>> 7) ^ (x << 14 | x >>> 18) ^ (x >>> 3))) | 0;
                    const y = schedule[j - 2];
                    temp = (temp + ((y << 15 | y >>> 17) ^ (y << 13 | y >>> 19) ^ (y >>> 10))) | 0;
                    schedule[j] = temp;
                }
                let [a, b, c, d, e, f, g, h] = this.state;
                for (let j = 0; j < schedule.length; j++) {
                    let t1 = (h + ((e << 26 | e >>> 6) ^ (e << 21 | e >>> 11) ^ (e << 7 | e >>> 25))) | 0;
                    t1 = (t1 + ((e & f) ^ (~e & g))) | 0;
                    t1 = (t1 + Sha256Core.K[j]) | 0;
                    t1 = (t1 + schedule[j]) | 0;
                    const t2 = (((a << 30 | a >>> 2) ^ (a << 19 | a >>> 13) ^ (a << 10 | a >>> 22)) + ((a & b) ^ (a & c) ^ (b & c))) | 0;
                    h = g;
                    g = f;
                    f = e;
                    e = (d + t1) | 0;
