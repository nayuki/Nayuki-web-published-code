...
"use strict";
/*---- HTML elements ----*/
let tableElem = queryElem("article form table");
...
let statusOut = queryElem("article #status output");
function getIoElems(rowId) {
    const input = queryElem(`article #${rowId} input`);
    if (input instanceof HTMLInputElement)
        return [input, getOutElems(rowId)];
    throw new TypeError();
}
function getOutElems(rowId) {
    const temp = document.querySelectorAll(`article #${rowId} output`);
    if (temp.length != 2)
        throw new Error();
    const [a, b] = temp;
...
function initImageMap() {
    let container = queryElem("article #diagram-container");
    const containerWidth = parseEm(container.style.width);
    let hovElem = queryElem("#diagram-container output");
