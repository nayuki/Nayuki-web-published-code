function solve(){
 try{
  ...
  if(sidesgiven==0)throw "At least one side must be given";
  if(sidesgiven+anglesgiven!=3)throw "Exactly 3 givens are required";
  var trianglecase;

  if(sidesgiven==3){ // SSS case
   trianglecase="Side side side (SSS)";
   angleA=cosLawSolveAngle(sideB,sideC,sideA);
   angleB=cosLawSolveAngle(sideC,sideA,sideB);
   angleC=cosLawSolveAngle(sideA,sideB,sideC);}

  else if(anglesgiven==2){ // ASA case
  ...
  else if(angleA!=null&&sideA==null||angleB!=null&&sideB==null||angleC!=null&&sideC==null){ // SAS case
   trianglecase="Side angle side (SAS)";
   if(sideA==null)sideA=cosLawSolveSide(sideB,sideC,angleA);
   if(sideB==null)sideB=cosLawSolveSide(sideC,sideA,angleB);
   if(sideC==null)sideC=cosLawSolveSide(sideA,sideB,angleC);
   if(angleA==null)angleA=cosLawSolveAngle(sideB,sideC,sideA);
   if(angleB==null)angleB=cosLawSolveAngle(sideC,sideA,sideB);
   if(angleC==null)angleC=cosLawSolveAngle(sideA,sideB,sideC);}
...
function cosLawSolveSide(a,b,C){ // Returns side c
 return Math.sqrt(a*a+b*b-2*a*b*Math.cos(degToRad(C)));}

function cosLawSolveAngle(a,b,c){ // Returns angle C
...
function getNumber(id){
 ...
 if(s=="")return null;
 var x=parseFloat(s);
 if(isNaN(x))throw "Not a real number";
 return x;}
